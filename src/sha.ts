...
function sha512Digest32(
  ...
  let H4_hi = initial[8],
    H4_lo = initial[9];
  let H5_hi = initial[10],
    H5_lo = initial[11];
  let H6_hi = initial[12],
    H6_lo = initial[13];
  let H7_hi = initial[14],
    H7_lo = initial[15];
  ...
  for (let offset = 0; offset < totalLen; offset += 128) {
    ...
    // Add to hash
    let carry = H0_lo + a_lo > 0xffffffff ? 1 : 0;
    H0_lo = (H0_lo + a_lo) >>> 0;
    H0_hi = ((H0_hi + a_hi) >>> 0) + carry;
    H0_hi = H0_hi >>> 0;
    H1_lo = (H1_lo + b_lo) >>> 0;
    carry = H1_lo < b_lo ? 1 : 0;
    H1_hi = ((H1_hi + b_hi) >>> 0) + carry;
    H1_hi = H1_hi >>> 0;
    H2_lo = (H2_lo + c_lo) >>> 0;
    carry = H2_lo < c_lo ? 1 : 0;
    H2_hi = ((H2_hi + c_hi) >>> 0) + carry;
    H2_hi = H2_hi >>> 0;
    H3_lo = (H3_lo + d_lo) >>> 0;
    carry = H3_lo < d_lo ? 1 : 0;
    H3_hi = ((H3_hi + d_hi) >>> 0) + carry;
    H3_hi = H3_hi >>> 0;
    H4_lo = (H4_lo + e_lo) >>> 0;
    carry = H4_lo < e_lo ? 1 : 0;
    H4_hi = ((H4_hi + e_hi) >>> 0) + carry;
    H4_hi = H4_hi >>> 0;
    H5_lo = (H5_lo + f_lo) >>> 0;
    carry = H5_lo < f_lo ? 1 : 0;
    H5_hi = ((H5_hi + f_hi) >>> 0) + carry;
    H5_hi = H5_hi >>> 0;
    H6_lo = (H6_lo + g_lo) >>> 0;
    carry = H6_lo < g_lo ? 1 : 0;
    H6_hi = ((H6_hi + g_hi) >>> 0) + carry;
    H6_hi = H6_hi >>> 0;
    H7_lo = (H7_lo + h_lo) >>> 0;
    carry = H7_lo < h_lo ? 1 : 0;
    H7_hi = ((H7_hi + h_hi) >>> 0) + carry;
    H7_hi = H7_hi >>> 0;
  }
  ...
  out[10] = (H1_hi >>> 8) & 0xff;
  out[11] = H1_hi & 0xff;
  out[12] = (H1_lo >>> 24) & 0xff;
  out[13] = (H1_lo >>> 16) & 0xff;
  out[14] = (H1_lo >>> 8) & 0xff;
  out[15] = H1_lo & 0xff;
  out[16] = (H2_hi >>> 24) & 0xff;
  out[17] = (H2_hi >>> 16) & 0xff;
  out[18] = (H2_hi >>> 8) & 0xff;
  out[19] = H2_hi & 0xff;
  out[20] = (H2_lo >>> 24) & 0xff;
  out[21] = (H2_lo >>> 16) & 0xff;
  out[22] = (H2_lo >>> 8) & 0xff;
  out[23] = H2_lo & 0xff;
  out[24] = (H3_hi >>> 24) & 0xff;
  out[25] = (H3_hi >>> 16) & 0xff;
  out[26] = (H3_hi >>> 8) & 0xff;
  out[27] = H3_hi & 0xff;
  out[28] = (H3_lo >>> 24) & 0xff;
  out[29] = (H3_lo >>> 16) & 0xff;
  out[30] = (H3_lo >>> 8) & 0xff;
  out[31] = H3_lo & 0xff;
  out[32] = (H4_hi >>> 24) & 0xff;
  out[33] = (H4_hi >>> 16) & 0xff;
  out[34] = (H4_hi >>> 8) & 0xff;
  out[35] = H4_hi & 0xff;
  out[36] = (H4_lo >>> 24) & 0xff;
  out[37] = (H4_lo >>> 16) & 0xff;
  out[38] = (H4_lo >>> 8) & 0xff;
  out[39] = H4_lo & 0xff;
  out[40] = (H5_hi >>> 24) & 0xff;
  out[41] = (H5_hi >>> 16) & 0xff;
  out[42] = (H5_hi >>> 8) & 0xff;
  out[43] = H5_hi & 0xff;
  out[44] = (H5_lo >>> 24) & 0xff;
  out[45] = (H5_lo >>> 16) & 0xff;
  out[46] = (H5_lo >>> 8) & 0xff;
  out[47] = H5_lo & 0xff;
  out[48] = (H6_hi >>> 24) & 0xff;
  out[49] = (H6_hi >>> 16) & 0xff;
  out[50] = (H6_hi >>> 8) & 0xff;
  out[51] = H6_hi & 0xff;
  out[52] = (H6_lo >>> 24) & 0xff;
  out[53] = (H6_lo >>> 16) & 0xff;
  out[54] = (H6_lo >>> 8) & 0xff;
  out[55] = H6_lo & 0xff;
  out[56] = (H7_hi >>> 24) & 0xff;
  out[57] = (H7_hi >>> 16) & 0xff;
  out[58] = (H7_hi >>> 8) & 0xff;
  out[59] = H7_hi & 0xff;
  out[60] = (H7_lo >>> 24) & 0xff;
  out[61] = (H7_lo >>> 16) & 0xff;
  out[62] = (H7_lo >>> 8) & 0xff;
  out[63] = H7_lo & 0xff;
